...
/* --------------------------------------------------------------------------
   Écran de chargement
   MIN_AFFICHAGE : durée minimale visible, le temps que l'animation se joue.
   Au-delà, l'écran s'efface dès que la page est prête. Mettez 3000 pour
   forcer 3 secondes pleines (déconseillé : ça fait fuir des visiteurs).
   -------------------------------------------------------------------------- */
(function initPreloader(){
  const MIN_AFFICHAGE = 1600; // ms
  const MAX_AFFICHAGE = 3000; // ms — plafond de sécurité

  const root = document.documentElement;
  if (!root.classList.contains('is-loading')) return;

  const debut = Number(root.dataset.loadStart) || Date.now();
  let fini = false;
  ...
  setTimeout(masquer, MAX_AFFICHAGE);
})();
